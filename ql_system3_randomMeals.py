...
class patient:
    def __init__(self,state):
        ...
        for s in self.state_space: # append current state to previous state
            for prev_u in self.state_space:
                for prev2_u in self.state_space:
                    self.hash.update({(s, prev_u, prev2_u) : i}) # append 2 time steps of previous action 
                    i += 1
    ...
    def sim_action(self, action):
        self.state[11] = self.state[9] # add 2 previous actions
