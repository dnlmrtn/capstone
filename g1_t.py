...
class patient:
    def __init__(self, state, sigma, error):
        self.t = 0
        self.last = -1
        self.state = state
        self.actions = []
        ...
        self.params = [291.0 + 291*error*random.gauss(0,sigma), 
                       3.17e-2 + 3.17e-2*error*random.gauss(0,sigma), 
                       1.23e-2 + 1.23e-2*error*random.gauss(0,sigma), 
                       2.9e-2 + 2.9e-2*error*random.gauss(0,sigma), 
                       9.0e-2 + 9.0e-2*error*random.gauss(0,sigma), 
                       1.2e-2 + 1.2e-2*error*random.gauss(0,sigma), 
                       1.8e-1 + 1.8e-1*error*random.gauss(0,sigma), 
                       8.00e-1 + 8.00e-1*error*random.gauss(0,sigma), 
                       12 + 12.0*error*random.gauss(0,sigma), 
                       12 + 12.0*error*random.gauss(0,sigma)]
        ...
        for g in self.state_space: #current state
            for time in range(len(self.meals)):
                self.hash.update({(g, time) : i}) 
                i += 1
        
        # initialize qtable values
        self.Q = np.zeros((len(self.state_space)*(len(self.meals)), len(self.action_space),2))
    ...
    def reward(self):
        ...
        if self.lower <= self.state[0] <= self.target:
            reward = (self.state[0] - self.lower)**3.0
            return reward
        if self.target < self.state[0] <= self.upper:
            reward  = -((self.target-self.lower)**3.0/(self.upper-self.target)**2.0)*((self.state[0]-self.target)**2.0)+((self.target-self.lower)**3.0)
            return reward
...
def qValUpdate(patient, action, gamma, lam):
    ...
    state1_curr_quantized = patient.state_space[index]
    index = (np.abs(patient.state_space - state2_curr)).argmin()
    state2_curr_quantized = patient.state_space[index]
    
    index = (np.abs(patient.state_space - state1_prev)).argmin()
    state1_prev_quantized = patient.state_space[index]
    index = (np.abs(patient.state_space - state2_prev)).argmin()
    state2_prev_quantized = patient.state_space[index]
    
    q_state1 = patient.hash[(state1_curr_quantized, time1)]
    q_state2 = patient.hash[(state2_curr_quantized, time2)]

    # get highest q at next state
    maxQ = patient.Q[q_state2, :, 0].max()

    # update using the Q learning equation
    qCurrent = patient.Q[q_state1,action,0]
    alpha = 1/patient.Q[q_state1,action,1]
    qNew = (1-alpha)*qCurrent + alpha*(patient.reward() + gamma*maxQ - qCurrent)
    patient.Q[q_state1,action,0] = qNew
    qDif = qNew - qCurrent

    # update state visits
    patient.Q[q_state1,action,1] += 1

    # Random exploration (were not in patients at this point)
    action2 = random.choice(patient.action_space)

    return qDif, state2, action2
...
def sim_test(patient, action, alpha, gamma, lam):
    # find what the next state is going to be given the current state and action
    state1_curr = patient.state[0] # current
    state1_prev = patient.state[7] # previous
    time1 = patient.t
    ...
    maxA = patient.Q[q_state2,:,0].argmax()
    maxQ = patient.Q[q_state2,:,0].max()
...
patient1 = patient(np.zeros(11), 0, 0.03)
patient1.state[0] = 80
patient1.state[1] = 30
patient1.state[2] = 30
patient1.state[3] = 17
...
for i in range(1000):
    qDif, patient1.state, action = sim_test(patient1, action, 0.1, 0.99, 0.1)

print('---sim done---')
ideal = [80]*len(patient1.glucose)
print(mean_squared_error(ideal, patient1.glucose))
